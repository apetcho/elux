...
Result Lynx::fn_log(const Vec<Self>& args){
    //! @todo: add doc-string of `math::log' to lynxDocs describing it syntax
    /*
        (math::log x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::log", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::log' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_log2(const Vec<Self>& args){
    //! @todo: add doc-string of `math::log2' to lynxDocs describing it syntax
    /*
        (math::log2 x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::log2", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::log2' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_log10(const Vec<Self>& args){
    //! @todo: add doc-string of `math::log10' to lynxDocs describing it syntax
    /*
        (math::log10 x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::log10", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::log10' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_log1p(const Vec<Self>& args){
    //! @todo: add doc-string of `math::log1p' to lynxDocs describing it syntax
    /*
        (math::log1p x)
    */
    Error err;
...
Result Lynx::fn_expm1(const Vec<Self>& args){
    //! @todo: add doc-string of `math::expm1' to lynxDocs describing it syntax
    /*
        (math::expm1 x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::expm1", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::expm1' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_sqrt(const Vec<Self>& args){
    //! @todo: add doc-string of `math::sqrt' to lynxDocs describing it syntax
    /*
        (math::sqrt x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::sqrt", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::sqrt' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_sin(const Vec<Self>& args){
    //! @todo: add doc-string of `math::sin' to lynxDocs describing it syntax
    /*
        (math::sin x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::sin", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::sin' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_tan(const Vec<Self>& args){
    //! @todo: add doc-string of `math::tan' to lynxDocs describing it syntax
    /*
        (math::tan x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::tan", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::tan' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_asin(const Vec<Self>& args){
    //! @todo: add doc-string of `math::asin' to lynxDocs describing it syntax
    /*
        (math::asin x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::asin", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::asin' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_acos(const Vec<Self>& args){
    //! @todo: add doc-string of `math::acos' to lynxDocs describing it syntax
    /*
        (math::acos x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::acos", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::acos' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_atan(const Vec<Self>& args){
    //! @todo: add doc-string of `math::atan' to lynxDocs describing it syntax
    /*
        (math::atan x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::atan", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::atan' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_erfc(const Vec<Self>& args){
    //! @todo: add doc-string of `math::erfc' to lynxDocs describing it syntax
    /*
        (math::erfc x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::erfc", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::erfc' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_tgamma(const Vec<Self>& args){
    //! @todo: add doc-string of `math::tgamma' to lynxDocs describing it syntax
    /*
        (math::tgamma x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::tgamma", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::tgamma' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_isinf(const Vec<Self>& args){
    //! @todo: add doc-string of `math::isinf?' to lynxDocs describing it syntax
    /*
        (math::isinf? x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::isinf?", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::isinf?' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_isnan(const Vec<Self>& args){
    //! @todo: add doc-string of `math::isnan?' to lynxDocs describing it syntax
    /*
        (math::isnan? x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::isnan?", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::isnan?' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_complex_real(const Vec<Self>& args){
    //! @todo: add doc-string of `math::complex.real' to lynxDocs describing it syntax
    /*
        (math::complex.real x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::complex.real", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::complex.real' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_complex_imag(const Vec<Self>& args){
    //! @todo: add doc-string of `math::complex.imag' to lynxDocs describing it syntax
    /*
        (math::complex.imag x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::complex.imag", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::complex.imag' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_complex_norm(const Vec<Self>& args){
    //! @todo: add doc-string of `math::complex.norm' to lynxDocs describing it syntax
    /*
        (math::complex.norm x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::complex.norm", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::complex.norm' to be a number";
        return Result(std::move(err));
    }
...
Result Lynx::fn_complex_conj(const Vec<Self>& args){
    //! @todo: add doc-string of `math::complex.conj' to lynxDocs describing it syntax
    /*
        (math::complex.conj x)
    */
    Error err;
    auto argc = args.size();
    auto pred = (argc==1);
    if(!Lynx::check_argc(pred, "math::complex.conj", err)){
        return Result(std::move(err));
    }
    if(!Lynx::check_type(args[0]->is_number(), args[0], err)){
        err.message() += "\nExpect the argument of `math::complex.conj' to be a number";
        return Result(std::move(err));
    }
